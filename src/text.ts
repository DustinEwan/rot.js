...
export class TextTokenizer {
	...
	private _breakLines(tokens, maxWidth) {
		...
		while (i < tokens.length) { /* take all text tokens, remove space, apply linebreaks */
			var token = tokens[i];
			if (token.type == Tokens.TYPE_NEWLINE) { /* reset */
				lineLength = 0; 
				lastTokenWithSpace = -1;
			}
			if (token.type != Tokens.TYPE_TEXT) { /* skip non-text tokens */
				i++;
				continue; 
			}
		...
		for (var i=0;i<tokens.length;i++) {
			var token = tokens[i];
			switch (token.type) {
				case Tokens.TYPE_TEXT: lastTextToken = token; break;
				case Tokens.TYPE_NEWLINE: 
